...
def load_model(ckpt_path: str, device: str = "cuda"):
    ...
    # Defaults (por si el checkpoint no guarda cfg)
    scale = 4
    embed_dim = 192
    depth = 12
    num_heads = 6
    ...
    # Fuerza "full-bit" si existe set_quant
...
def tensor_to_pil(t: torch.Tensor) -> Image.Image:
    t = t.squeeze(0).clamp(0, 1).permute(1, 2, 0).detach().cpu().numpy()
...
def tile_inference(model, lr, scale=4, tile=96, overlap=8, device="cuda"):
    """
    lr: (1,3,H,W) float32 en GPU
    Hace autocast FP16 solo durante el forward del modelo.
    """
    model.eval()
    ...
    # acumuladores en FP32 (más estable, y no te rompe dtype)
    ...
    while y < H:
        ...
        while x < W:
            ...
            # FP16 solo en inferencia, sin convertir el modelo a half
...
def run_sr(model, img_in: Image.Image, device="cuda", tile=96, overlap=8):
...
def main():
    ...
    if args.web:
        ...
        demo = gr.Interface(
            ...
            description="Inferencia con autocast FP16 (sin convertir el modelo). Ajusta tile/overlap si va lento u OOM.",
            flagging_mode="never",
        )
        demo.launch(server_name=args.host, server_port=args.port)
        return

    raise SystemExit("Usa --web o --in/--out")
